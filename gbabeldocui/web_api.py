...
def _build_engine_settings(service: str, user_settings: dict):
    ...
    if normalized_service == "SiliconFlow":
        return SiliconFlowSettings(
            ...
            siliconflow_base_url=user_settings.get(
    ...
    if normalized_service in ("Claude", "ClaudeCode"):
        return ClaudeCodeSettings(
            ...
            claude_code_path=user_settings.get("claude_code_path", "claude"),
        )
...
def build_settings_model_from_user_config(
    ...
    engine_settings = _build_engine_settings(service, user_settings)
    ...
    settings.pdf.translate_table_text = user_settings.get(
        'translate_tables',
        user_settings.get('translate_table_text', False),
    )
...
async def register_user(request: RegisterRequest, _admin_user: dict = Depends(get_admin_user)):
...
async def import_settings(
    ...
    try:
        ...
        if len(content) > MAX_UPLOAD_BYTES:
            raise HTTPException(
                status_code=413,
                detail=f"Configuration file exceeds the {MAX_UPLOAD_BYTES} byte limit",
            )
        ...
        if not isinstance(imported_settings, dict):
            raise HTTPException(status_code=400, detail="Invalid configuration file: 'settings' must be an object")
...
async def start_translation(
    ...
    if not isinstance(translation_settings, dict):
        raise HTTPException(status_code=400, detail="Translation settings must be an object")
    ...
    matching_file = next(
        (candidate for candidate in upload_dir.glob(f"{file_id}_*") if candidate.is_file()),
        None,
    )
